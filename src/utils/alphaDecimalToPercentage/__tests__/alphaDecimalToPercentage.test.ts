...
describe("alphaDecimalToPercentage", () => {
	...
	it("should throw error for out of range alpha values", () => {
		expect.hasAssertions();
		expect(() => alphaDecimalToPercentage(-0.1)).toThrow(
			ERROR_MESSAGES.invalidAlphaRange,
		);
		expect(() => alphaDecimalToPercentage(1.1)).toThrow(
			ERROR_MESSAGES.invalidAlphaRange,
		);
	});
	...
	it("should throw error for non-numeric values", () => {
		expect.hasAssertions();
		expect(() =>
			alphaDecimalToPercentage("blabla" as unknown as number),
		).toThrow(
		...
		expect(() =>
			alphaDecimalToPercentage(undefined as unknown as number),
		).toThrow(
			ERROR_MESSAGES.invalidColorType({

...
describe("extractHslaObjectFromString", () => {
		...
		const testCases = [
			{
				...
				expected: { h: 180, s: 50, l: 50, a: 0.5 },
			},
			{ input: "hsla(0, 0%, 0%, 0)", expected: { h: 0, s: 0, l: 0, a: 0 } },
			{
				input: "hsla(360, 100%, 100%, 1)",
				expected: { h: 360, s: 100, l: 100, a: 1 },
			},
			{
				input: "hsla(45, 75%, 25%, 0.25)",
				expected: { h: 45, s: 75, l: 25, a: 0.25 },
			},
		];
	...
	it("should throw error for invalid HSLA format", () => {
		...
		invalidInputs.forEach((input) => {
			expect(() => extractHslaObjectFromString(input)).toThrow(
				ERROR_MESSAGES.invalidColorFormat({
					expectedFormatRegexp: HSLA_COLOR_REGEX,
				}),
			);
		});
	});
	...
	it("should throw error for non-string input", () => {
		...
		invalidInputs.forEach((input) => {
			expect(() =>
				extractHslaObjectFromString(input as unknown as string),
			).toThrow(

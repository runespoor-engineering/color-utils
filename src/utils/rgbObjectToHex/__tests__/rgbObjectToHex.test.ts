...
describe("rgbObjectToHex", () => {
	...
	it("should throw error for out of range RGB values", () => {
		expect.hasAssertions();
		expect(() => rgbObjectToHex({ r: -1, g: 0, b: 0 })).toThrow(
			ERROR_MESSAGES.invalidRgbRange,
		);
		expect(() => rgbObjectToHex({ r: 256, g: 0, b: 0 })).toThrow(
			ERROR_MESSAGES.invalidRgbRange,
		);
		expect(() => rgbObjectToHex({ r: 0, g: -1, b: 0 })).toThrow(
			ERROR_MESSAGES.invalidRgbRange,
		);
		expect(() => rgbObjectToHex({ r: 0, g: 256, b: 0 })).toThrow(
			ERROR_MESSAGES.invalidRgbRange,
		);
		expect(() => rgbObjectToHex({ r: 0, g: 0, b: -1 })).toThrow(
			ERROR_MESSAGES.invalidRgbRange,
		);
		expect(() => rgbObjectToHex({ r: 0, g: 0, b: 256 })).toThrow(
			ERROR_MESSAGES.invalidRgbRange,
		);
	});
	...
	it("should throw error for invalid input types", () => {
		...
		expect(() =>
			rgbObjectToHex("not an object" as unknown as RgbObject),
		).toThrow(
			ERROR_MESSAGES.invalidColorType({
				actualType: "string",
				expectedType: "object",

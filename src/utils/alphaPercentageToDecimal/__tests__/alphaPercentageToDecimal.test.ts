...
describe("alphaPercentageToDecimal", () => {
	...
	it("should throw error for out of range percentage values", () => {
		expect.hasAssertions();
		expect(() => alphaPercentageToDecimal(-10)).toThrow(
			ERROR_MESSAGES.invalidAlphaRange,
		);
		expect(() => alphaPercentageToDecimal(110)).toThrow(
			ERROR_MESSAGES.invalidAlphaRange,
		);
	});
	...
	it("should throw error for non-numeric values", () => {
		expect.hasAssertions();
		expect(() =>
			alphaPercentageToDecimal("blabla" as unknown as number),
		).toThrow(
		...
		expect(() =>
			alphaPercentageToDecimal(undefined as unknown as number),
		).toThrow(
			ERROR_MESSAGES.invalidColorType({
